...
fragment_img_1 = image_loader("./data/Seg1.jpg")
fragment_img_2 = image_loader("./data/Seg2.jpg")
...
plt.figure()
imshow(fragment_img_1, title='Style Image')

plt.figure()
imshow(fragment_img_2, title='Content Image')
...
input_img = torch.randn(fragment_img_1.data.size(), device=device)
...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std,
    ...
    for layer in cnn.children():
        ...
        if name in content_layers:
            # add content loss:
            target1 = model(seg1_img).detach()
            content_loss1 = ContentLoss(target1)
            model.add_module("content_loss1_{}".format(i), content_loss1)
            seg1_content_losses.append(content_loss1)

        if name in content_layers:
            # add content loss:
            target2 = model(seg2_img).detach()
            content_loss2 = ContentLoss(target2)
            model.add_module("content_loss2_{}".format(i), content_loss2)
            seg2_content_losses.append(content_loss2)
        '''
    ...
    return model, seg1_content_losses, seg2_content_losses
...
def run_style_transfer(cnn, normalization_mean, normalization_std,
                       fragment_img_1, fragment_img_2, input_img, num_steps=300,
                       seg_content_weight=1):
    """Run the style transfer."""
    print('Building the style transfer model..')
    model, seg1_content_losses, seg2_content_losses = get_style_model_and_losses(cnn,
        normalization_mean, normalization_std, fragment_img_1, fragment_img_2)
    ...
    while run[0] <= num_steps:
        ...
        def closure():
            ...
            optimizer.zero_grad()
            model(input_img)
            seg1_content_score = 0
            seg2_content_score = 0
            ...
            for cl in seg1_content_losses:
                seg1_content_score += cl.loss
            for cl in seg2_content_losses:
                seg2_content_score += cl.loss

            seg1_content_score *= seg_content_weight
            seg2_content_score *= seg_content_weight

            loss = seg1_content_score + seg2_content_score
            loss.backward()
            ...
            if run[0] % 50 == 0:
                ...
                print('Style Loss : {:4f} Content Loss: {:4f}'.format(
                    seg1_content_score.item(), seg2_content_score.item()))
                print()

            return seg1_content_score + seg2_content_score
...
output = run_style_transfer(cnn, cnn_normalization_mean, cnn_normalization_std,
                            fragment_img_1, fragment_img_2, input_img)
